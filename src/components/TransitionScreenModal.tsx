 ...
 * but can't be satisfied from the TUI — submit is blocked with a toast
 * telling the user to complete the transition in the browser.
 */
export function TransitionScreenModal({
  ...
  const [values, setValues] = useState<Record<string, TransitionFieldValue>>({});
  ...
  const openEditor = useCallback(
    async (field: TransitionField) => {
      if (field.kind === "unsupported") {
        onError(`${field.name} can't be edited from the TUI — complete in browser`);
        return;
      }
      ...
      if (field.kind === "user" || field.kind === "user-list") {
        ...
        try {
          ...
          setSub({ kind: field.kind, field: field as any, users });
        } catch (e) {
  ...
  const doSubmit = useCallback(() => {
    ...
    onSubmit(values);
  }, [missing, values, onSubmit, onError]);
  ...
  useInput(
    ...
    { isActive: sub.kind === "none" && !loading },
  );
      ...
      <InlineFieldInput
        ...
        onSubmit={(raw) => {
          ...
          if (sub.kind === "number") {
            ...
            if (Number.isNaN(n)) {
              onError(`${sub.field.name}: invalid number`);
              return;
            }
            setValues((v) => ({ ...v, [sub.field.id]: n }));
          } else if (sub.kind === "date") {
            if (trimmed !== "" && !/^\d{4}-\d{2}-\d{2}$/.test(trimmed)) {
              onError(`${sub.field.name}: use YYYY-MM-DD`);
              return;
            }
            if (trimmed === "") {
              const next = { ...values };
